...
p1 <- ggplot() + 
  ...
  scale_fill_viridis_c(name = "Prevalence", trans = "sqrt") +
  scale_size_continuous(name = "Sample size", range = c(0.2, 6), trans = "sqrt") +
  labs(title = "Prevalence of Kelch 13 markers since 2020") +
...
p2 <- ggplot() +
  ...
  theme_classic() +
  labs(title = "Estimated Kelch 13 prevalence (2024)") +
  theme(axis.title = element_blank(), 
        axis.text = element_blank(), 
        axis.ticks = element_blank(),
        axis.line = element_blank(),
...
p3 <- ggplot() +
  ...
  labs(title = "Spread of Kelch 13 mutations over time") +
  theme_classic() +
  theme(axis.title = element_blank(), 
        axis.text = element_blank(), 
        axis.ticks = element_blank(),
...
ggsave("figures/policy_k13.png", p, height = 12, width = 6, scale = 1.2)
...
pcol <- plot_grid(p1 + theme(legend.position = "none", plot.margin = unit(rep(0,4), "cm")), 
                  ...
                  ncol = 1, rel_heights = c(1.2, rep(1, 7))) +
...
df <- data.frame(xmin = rep(0.001, 4),
                 xmax = rep(0.035, 4),
                 ymin = seq(0.015, 0.726, length.out = 4),
                 ymax = seq(0.235, 0.95, length.out = 4),
...
p <- p + 
  ...
  geom_text(data = data.frame(x = 0.85, y = c(0.7), 
                              label = c("Prevalence")),
            aes(x = x, y = y, label = label))

ggsave("figures/policy_partners.png", p, height = 5, width = 7.2)
